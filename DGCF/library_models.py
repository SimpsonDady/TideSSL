...
class DGCF(nn.Module):
    def __init__(self, args, num_features, num_users, num_items):
        ...
        self.user_estimator = nn.Linear(self.embedding_dim, self.embedding_dim)
        self.item_estimator = nn.Linear(self.embedding_dim, self.embedding_dim)
    ...
    def forward(self, user_embeddings, item_embeddings, timediffs=None,  features=None, adj_embeddings=None, select=None):
        ...
        #     return F.normalize(user_embedding_output)
#         if features != None:
#             features = self.micro_network(features).reshape(len(features), -1)
    ...
    def micro_network(self, feature):
        cross_features = torch.tensor([]).cuda()
        for i in range(len(feature)):
            cross = torch.triu(torch.ger(feature[i], feature[i]), diagonal=1).flatten()
            cross_features = torch.cat([cross_features, cross[cross.nonzero(as_tuple=False)].flatten(1)], axis=1)
        cross_features = torch.cat([feature, cross_features.reshape(len(feature), -1)], axis=1)
...
def pretext(p_m, x):
    mask = torch.Tensor(np.random.binomial(1, p_m, x.shape)).cuda()
    
    no, dim = x.shape
    x_bar = torch.zeros(no, dim, dtype=torch.float32).cuda()
    for i in range(dim):
    ...
    return mask_new.type(torch.float32), x_tilde
