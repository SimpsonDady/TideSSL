...
How to run: 
$ python3 DGCF.py --network reddit --model DGCF --epochs 50
...
parser.add_argument('--alpha', type=float, default=0.0, help='SSL dropout rate')
...
with trange(args.epochs) as progress_bar1:
    ...
    for ep in progress_bar1:
        if ep == args.epochs // 2:
            supervised = 1
        progress_bar1.set_description('Epoch %d of %d, Supervised: %s' % (ep, args.epochs, bool(supervised)))
        ...
        with trange(train_end_idx) as progress_bar2:
            for j in progress_bar2:
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    drop_indices = np.random.choice(range(n_batches), int(n_batches*args.alpha), replace=False)
                    if n_batches < 2:
                        continue
                    ...
                    for i in range(n_batches):
                        ...
                        main_loss += args.l2i*MSELoss(item_embedding_output, item_embedding_input.detach())
                        main_loss += args.l2u*MSELoss(user_embedding_output, user_embedding_input.detach())
                        ...
                        if i in drop_indices:
                            ...
                            self_user_embedding_output = model.forward(user_embedding_input, item_embedding_input,
                                                                  timediffs=user_timediffs_tensor, features=feature_tensor,
                                                                  adj_embeddings=user_adj_embedding, select='user_update')
                            self_item_embedding_output = model.forward(user_embedding_input, item_embedding_input,
                                                                  timediffs=item_timediffs_tensor, features=feature_tensor,
                                                                  adj_embeddings=item_adj_embedding, select='item_update')
