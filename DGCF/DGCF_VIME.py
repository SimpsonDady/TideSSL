...
parser.add_argument('--pm', default=0.3, type=float, help='corruption probability for self-supervised learning')
            ...
            for j in progress_bar2:
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    for i in range(len(lib.current_tbatches_user)):
                        ...
                        user_mask, user_embedding_input = pretext(args.pm, user_embeddings[tbatch_userids,:])
                        ...
                        item_mask, item_embedding_input = pretext(args.pm, item_embeddings[tbatch_itemids,:])
                        ...
                        estimated_user_embedding_output = torch.sigmoid(model.user_estimator(user_embedding_output))
                        estimated_item_embedding_output = torch.sigmoid(model.item_estimator(item_embedding_output))

                        item_embeddings[tbatch_itemids, :] = item_embedding_output
                        user_embeddings[tbatch_userids, :] = user_embedding_output
                        
#                         print(user_mask)
#                         print(estimated_user_embedding_output)
                        ...
                        loss += BCELoss(estimated_user_embedding_output, user_mask.detach())
                        loss += BCELoss(estimated_item_embedding_output, item_mask.detach())
