...
$ python3 evaluate_interaction_prediction.py --network reddit --model DGCF --epoch 49
...
parser.add_argument('--alpha', type=str, default="", help='SSL dropout rate')
...
if args.adj:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.%s_%s.txt" % (args.network, args.method, 'adj', args.alpha, time)
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.%s_%s.txt" % (args.network, args.method, args.l2u, args.l2i,'adj', args.alpha, time)
elif args.no_zero:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.%s_%s.txt" % (args.network, args.method, 'no_zero', args.alpha, time)
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.%s_%s.txt" % (args.network, args.method, args.l2u, args.l2i,'no_zero', args.alpha, time)
elif args.no_first:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.%s_%s.txt" % (args.network, args.method, 'no_first', args.alpha, time)
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.%s_%s.txt" % (args.network, args.method, args.l2u, args.l2i,'no_first', args.alpha, time)
else:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s_%s.txt" % (args.network, args.method, args.alpha, time)
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s_%s.txt" % (args.network, args.method, args.l2u, args.l2i, args.alpha, time)
