...
How to run: 
$ python3 tbatch.py --network lastfm 
...
fout.write("tbatch_id,interactionids,user_id,item_id,timestamp,state_label,user_timediffs,item_timediffs,previous_items,user_adj,item_adj")
...
for j in range(num_interactions):
    ...
    if timestamp - tbatch_start_time > tbatch_timespan or j == num_interactions - 1:
        ...
        for tidx in range(len(lib.current_tbatches_user)):
            ...
            tbatches_user_timediffs = lib.current_tbatches_user_timediffs[tidx]
            tbatches_item_timediffs = lib.current_tbatches_item_timediffs[tidx]
            tbatches_previous_items = lib.current_tbatches_previous_item[tidx]
            
            tbatch_user_adjs = lib.current_tbatches_user_adj
            tbatch_item_adjs = lib.current_tbatches_item_adj

            batch = zip(tbatch_interactionids, tbatch_userids, tbatch_itemids, tbatch_timestamps, tbatch_labels, tbatch_features, tbatches_user_timediffs, tbatches_item_timediffs, tbatches_previous_items, tbatch_user_adjs, tbatch_item_adjs)
            for interid, uid, iid, ts, lbl, feature, udf, idf, pi, uadj, iadj in batch:
                arr = map(str, [tbatchID, interid, uid, iid, ts, lbl, udf, idf, pi, uadj, iadj] + feature)
                fout.write(",".join(arr) + "\n")
