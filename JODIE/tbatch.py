...
fout.write("tbatch_id,interactionids,user_id,item_id,timestamp,state_label,user_timediffs,item_timediffs,previous_items")
...
for j in range(num_interactions):
    ...
    if timestamp - tbatch_start_time > tbatch_timespan or j == num_interactions - 1:
        ...
        for tidx in range(len(lib.current_tbatches_user)):
            ...
            batch = zip(tbatch_interactionids, tbatch_userids, tbatch_itemids, tbatch_timestamps, tbatch_labels, tbatch_features, tbatches_user_timediffs, tbatches_item_timediffs, tbatches_previous_items)
            for interid, uid, iid, ts, lbl, feature, udf, idf, pi in batch:
                arr = map(str, [tbatchID, interid, uid, iid, ts, lbl, udf, idf, pi] + feature)
                fout.write(",".join(arr) + "\n")
            print(tidx, len(lib.current_tbatches_interactionids[tidx]), end=' | ')
