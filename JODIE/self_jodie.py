...
How to run: 
$ python3 self_jodie.py --network reddit --model jodie --epochs 50
...
def softmax_cross_entropy(A, B):
    ...
    cosine = torch.mm(A, B.T) / torch.mm(A_norm, B_norm.T)
    cosine_exp = torch.exp(cosine / 0.2)
    cosine_exp_sum = torch.sum(cosine_exp, dim=1, keepdims=True)
    softmax = -torch.mean(torch.log(cosine_exp / cosine_exp_sum), dim=1, keepdims=True)
    return 0.1 * torch.sum(softmax)
...
with trange(args.epochs) as progress_bar1:
    for ep in progress_bar1:
        ...
        total_loss, main_loss, self_loss, total_interaction_count = 0, 0, 0, 0
...
#                     print(lib.current_tbatches_user)
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    n_batches = len(lib.current_tbatches_user)
                    drop_indices = np.random.choice(range(n_batches), int(n_batches*0.3), replace=False)
#                     if n_batches < 2:
#                         print("pass", n_batches)
#                         continue
                    ...
                    for i in range(len(lib.current_tbatches_user)):
                        ...
                        try:
                            ...
                            print(lib.current_tbatches_feature[i])
                            raise
                        ...
                        main_loss += MSELoss(predicted_item_embedding, torch.cat([item_embedding_input, item_embedding_static[tbatch_itemids,:]], dim=1).detach())
                        ...
                        main_loss += MSELoss(item_embedding_output, item_embedding_input.detach())
                        main_loss += MSELoss(user_embedding_output, user_embedding_input.detach())
                        ...
                        if i in drop_indices:
                            ...
                            user_embedding_input = self_user_embeddings[tbatch_userids,:]
                            item_embedding_input = self_item_embeddings[tbatch_itemids,:]
                            
                            self_user_embedding_output = model.forward(user_embedding_input, item_embedding_input, timediffs=user_timediffs_tensor, features=feature_tensor, select='user_update')
                            self_item_embedding_output = model.forward(user_embedding_input, item_embedding_input, timediffs=item_timediffs_tensor, features=feature_tensor, select='item_update')

                            self_user_embeddings[tbatch_userids, :] = self_user_embedding_output
                            self_item_embeddings[tbatch_itemids, :] = self_item_embedding_output
                            self_user_embeddings_timeseries[tbatch_interactionids,:] = self_user_embedding_output
                            self_item_embeddings_timeseries[tbatch_interactionids,:] = self_item_embedding_output

                    self_loss += softmax_cross_entropy(self_user_embeddings.detach_(), user_embeddings)
                    self_loss += softmax_cross_entropy(self_item_embeddings.detach_(), item_embeddings)
                    
                    # BACKPROPAGATE ERROR AFTER END OF T-BATCH
#                     loss = supervised * main_loss + (1 - supervised) * self_loss
                    loss = main_loss + self_loss
#                     print("Self_loss: %.2f" % self_loss.item())
                    ...
                    self_user_embeddings = user_embeddings
                    self_item_embeddings = item_embeddings
        ...
        self_user_embeddings = initial_user_embedding.repeat(num_users, 1)
        self_item_embeddings = initial_item_embedding.repeat(num_items, 1)
